...
async function generatePrintroveToken() {
  try {
    const response = await axios.post(
      `${process.env.PRINTROVE_BASE_URL}/external/token`, // ✅ must include /api in env base url
      {
    ...
    if (!access_token) throw new Error("Missing access_token from Printrove response");
    ...
    console.error("❌ Printrove token generation failed:", err.response?.data || err.message);
